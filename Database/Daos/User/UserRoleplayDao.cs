...
namespace WibboEmulator.Database.Daos
{
    class UserRoleplayDao
    {
        ...
        internal static void Update(IQueryAdapter dbClient, int userId, int roleplayId, int health, int energy, int money, int munition, int exp, int weaponGunId, int weaponCacId)
        {
            dbClient.RunQuery("UPDATE `user_roleplay` SET `health` = '" + health + "', `energy` = '" + energy + "', `money` = '" + money + "', `munition` = '" + munition + "', `exp` = '" + exp + "', `weapon_far` = '" + weaponGunId + "', `weapon_cac` = '" + weaponCacId + "' WHERE `user_id` = '" + userId + "' AND `roleplay_id` = '" + roleplayId + "' LIMIT 1");
        }
