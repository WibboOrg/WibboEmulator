...
namespace Butterfly.Database.Daos
{
    class ItemWiredDao
    {
        ...
        {
            dbClient.RunQuery("INSERT INTO `item_wired` (trigger_id, trigger_data_2, trigger_data, all_user_triggerable, triggers_item) " +
                                 "SELECT '" + itemId + "', trigger_data_2, trigger_data, all_user_triggerable, triggers_item FROM `item_wired` WHERE trigger_id = '" + oldItemId + "'");
        }

        internal static DataRow GetOne(IQueryAdapter dbClient, int triggerId)
        {
            dbClient.SetQuery("SELECT trigger_data, trigger_data_2, triggers_item, all_user_triggerable FROM `item_wired` WHERE trigger_id = @id");
        ...
        internal static void Insert(IQueryAdapter dbClient, int triggerId, string triggerData, string triggerData2, bool allUsertriggerable, string triggersitem)
        {
            dbClient.SetQuery("INSERT INTO `item_wired` (trigger_id, trigger_data, trigger_data_2, all_user_triggerable, triggers_item) VALUES (@id, @trigger_data, @trigger_data_2, @triggerable, @triggers_item)");
            dbClient.AddParameter("id", triggerId);
