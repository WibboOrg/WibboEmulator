...
namespace Butterfly.Database.Daos
{
    class CatalogMarketplaceOfferDao
    {
        ...
        {
            dbClient.SetQuery("SELECT asking_price FROM catalog_marketplace_offers WHERE user_id = '" + userId + "' AND state = '2'");
            return dbClient.GetTable();
        }

        internal static void Delete(IQueryAdapter dbClient, int userId)
        {
            dbClient.RunQuery("DELETE FROM catalog_marketplace_offers WHERE user_id = '" + userId + "' AND state = '2'");
        }
