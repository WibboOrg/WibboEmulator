...
internal sealed class Turn : IChatCommand
{
    public void Execute(GameClient session, Room room, RoomUser userRoom, string[] parameters)
    {
        ...
        userRoom.OnChat(string.Format(WibboEnvironment.GetLanguageManager().TryGetValue("cmd.turn.chat", session.Langue), targetUser.GetUsername()));
        targetUser.OnChat(string.Format(WibboEnvironment.GetLanguageManager().TryGetValue("cmd.turn.chat.target", session.Langue), userRoom.GetUsername()));
